...
class YoLov5TRT(object):
    ...
    def infer(self, raw_image_generator, vis=True):
        ...
        for i, image_raw in enumerate(raw_image_generator):
            ...
            np.copyto(batch_input_image[i], input_image) #将imput_image数组元素复制到batch_input_image中
        batch_input_image = np.ascontiguousarray(batch_input_image)#将一个内存不连续存储的数组转换为内存连续存储的数组
            ...
            if results[0] is not None:
                if vis:  # 可视化
                    ...
                    for j in range(len(top_boxes)):
                        box = top_boxes[j]
                        class_idx = int(top_label[j])
                        color = self.colors[class_idx]  # 获取类别对应的颜色
                        # plot_one_box(box,batch_image_raw[i],label="{}:{:.2f}".format(categories[int(top_label[j])], top_conf[j]),)
                        label = "{}:{:.2f}".format(categories[class_idx], top_conf[j])
                        plot_one_box(box, batch_image_raw[i], color=color, label=label)
            else:
